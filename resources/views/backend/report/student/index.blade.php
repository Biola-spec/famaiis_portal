        ...
        <section class="content">
            <div class="row">
                <div class="col-md-10 offset-md-1">
                    <h2 class="mb-4">My Activity Reports</h2>

                    <div class="box mb-4">
                        <div class="box-body">
                            <form action="{{ route('student.report.index') }}" method="GET" class="row">
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <a href="{{ route('student.report.index') }}" class="btn btn-secondary btn-block">Clear Filters</a>
                                </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <div class="card report-card">
                            <div class="report-header">
                                ...
                                <div class="teacher-info">
                                    <h5>{{ optional($report->teacher)->name }}</h5>
                                    <div class="report-meta">
                                        <span>{{ optional($report->approved_at ?? $report->created_at)->diffForHumans() }}</span> |
                                        <span>{{ optional($report->studentClass)->name }}</span>
                                        @if($report->subject) | <span>{{ $report->subject->name }}</span> @endif
                                    </div>
                                </div>
                                <span class="badge badge-info ml-auto">{{ ucfirst($report->report_type) }}</span>
                            </div>
                    ...
                    @empty
                        <div class="text-center py-5">
                            <h4 class="mt-4 text-muted">No approved activity reports yet.</h4>
                        </div>
                    @endforelse
