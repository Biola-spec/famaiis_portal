...
<div class="row mb-4">
    <div class="col-md-12">
        ...
        <div id="children_list_container">
            <table class="table table-bordered table-striped" id="selected_children_table" style="{{ $editData->children->count() > 0 ? '' : 'display: none;' }}">
                <thead>
                    <tr>
                        <th>Student Name</th>
                        <th>ID Number</th>
                ...
                <tbody id="selected_children_tbody">
                    @foreach($editData->children as $child)
                        @php
                            $latestAssignment = $child->studentClassAssignments->sortByDesc('id')->first();
                            $childClassName = $latestAssignment && $latestAssignment->student_class ? $latestAssignment->student_class->name : 'No Assigned Class';
                        @endphp
                        <tr id="child-row-{{ $child->id }}">
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-danger btn-sm remove-child-btn" data-id="{{ $child->id }}" style="padding: 2px 8px !important; font-size: 11px !important;">
            ...
            <div id="no_children_alert" class="alert alert-secondary text-center py-3" style="background: var(--bg-body); border: 1px dashed var(--card-border); color: var(--text-muted); border-radius: 8px; {{ $editData->children->count() > 0 ? 'display: none;' : '' }}">
