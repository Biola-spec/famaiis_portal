...
+function ($) {
  ...
  Layout.prototype.fixSidebar = function () {
    // Save current scroll position before destroying/recreating
    var currentScroll = 0;
    if (typeof $.fn.slimScroll !== 'undefined' && $(Selector.sidebar).length > 0) {
      currentScroll = $(Selector.sidebar).scrollTop();
    }
    ...
    if ($(window).width() <= 767) {
      ...
      if (currentScroll > 0) {
        $(Selector.sidebar).scrollTop(currentScroll)
      }
    ...
    if (this.options.slimscroll) {
      if (typeof $.fn.slimScroll !== 'undefined') {
        ...
        if (currentScroll > 0) {
          $(Selector.sidebar).slimScroll({ scrollTo: currentScroll + 'px' });
        }
      }
    }
  }
